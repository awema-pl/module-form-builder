...
export const getters = {
    ...
    errorsOrFalse: (state, getters) => name => {
        ...
        return AWEMA.utils.object.isEmpty(errors) ? false : errors
    },
...
export const mutations = {
    ...
    createField(state, { formName, fieldName, value }) {
        const form = state[formName]
        value = AWEMA.utils.object.get(form.initialState, fieldName, value)
    ...
    createMutiblock(state, { formName, multiblockName, disabled }) {
        const form = state[formName]
        const { get, isEmpty } = AWEMA.utils.object
    ...
    restoreData({ state }, { formName }) {
        ...
        if (multiblockNames.length) {
            ...
            for (let multiblockName of multiblockNames) {
                let ids = Object.keys(AWEMA.utils.object.get(data, multiblockName, [{}])).map(id => Number(id))
    ...
    sendForm({ state, commit, dispatch }, {formName, url, method}) {
        ...
        return new Promise( resolve => {
            ...
            dispatch('restoreData', { formName })
                .then( data => {
                    // send request
                    return AWEMA.ajax(data, url, method)
                })
                .then( res => {
                    _res = res
                    ...
                    if ( res.success ) {
                        ...
                        for (let field in form.fields) {
                            _data[field] = AWEMA.utils.object.get(data, field)
                        }
