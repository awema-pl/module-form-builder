...
export default {
        ...
        replacedUrl() {
            ...
            if ( this.form ) {
                ...
                props && props.length && props.forEach( prop => {
                    url = url.replace('{' + prop + '}', AWEMA.utils.object.get(this.form.initialState, prop, ''))
                })
            }
    ...
    methods: {
        ...
        send() {
            ...
            AWEMA.emit(`form-builder::${this.name}:before-send`)
        ...
        close() {
            if ( this.checkCloseAllowed() ) {
                ...
                AWEMA.off(`modal::${this.modal.name}:before-close`, this.preventModalClose)
                this.modal.close()
            }
        },
    ...
    created() {
        ...
        if ( this.modal ) {
            this.$watch('isEdited', edited => {
                AWEMA[edited ? 'on': 'off'](`modal::${this.modal.name}:before-close`, this.preventModalClose)
            })
        }
    },
