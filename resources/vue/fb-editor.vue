...
export default {
    ...
    methods: {
        ...
        initEditor() {
            ...
            let options = AWEMA.utils.object.get(AWEMA._config, 'formBuilder.fbEditor', {})
            ...
            if ( typeof AWEMA._theme !== undefined ) {
                editor.once('Init', () => {
                    this._switchThemeAttribute({detail: AWEMA._theme})
                })
            }
        },
    ...
    mounted() {
        if ( this.formId ) {
            AWEMA.on(`form-builder::${this.formId}:before-send`, this.save)
        }
        this.$nextTick( this.initEditor )
        if ( typeof AWEMA._theme !== undefined ) {
            AWEMA.on('theme.change', this._switchThemeAttribute)
        }
    },
    ...
    beforeDestroy() {
        if ( this.formId ) {
            AWEMA.off(`form-builder::${this.formId}:before-send`, this.save)
        }
        if ( typeof AWEMA._theme !== undefined ) {
            AWEMA.off('theme.change', this._switchThemeAttribute)
        }
    }
}
</script>
